...
export default function MealButton({ mealNumber, meal_id, mealTaken, admin_id }: {
    ...
    return (
        <button
            disabled={mealTaken}
            onClick={takeMeal}
            className={`border w-full px-4 py-2 rounded-md shadow-sm duration-300 ease-in-out flex flex-row justify-center items-center ${mealTaken ? 'bg-red-500 text-white line-through' : 'hover:bg-gray-300'}`}
        >
