    ...
    return (
        <div>
            ...
            <div className={'container flex flex-col items-center gap-4'}>
                <h1 className={'text-center w-1/2'}>{`Name: ${user.applicant_details[0].first_name} ${user.applicant_details[0].last_name}`}</h1>
                <CheckinButton checkedIn={checkedIn} checkInData={checkInData} />
                {[1,2,3,4,5].map((meal) => {
                    return <MealButton key={meal} mealNumber={meal} meal_id={meals![meal-1].meal_id} mealTaken={meals![meal-1].meal_taken} admin_id={admin_id?.admin_id} />
                })}
            </div>
        </div>
    );
